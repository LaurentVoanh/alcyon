...
if ($phase === 'reply') {
    ...
    foreach ($history as $h) {
        $messages_ctx[] = ['role' => $h['role'], 'content' => $h['content']];
    }
    ...
    if (!$res['raw'] || $res['code'] !== 200) {
        ...
        echo json_encode(['error' => ($res['err'] ?: "HTTP {$res['code']}") . ($detail ? " — $detail" : ''), 'timestamp' => date('H:i:s')], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    if ($msg_count > 0 && $msg_count % 5 === 0) {
        ...
        $ctx_res = do_curl(MISTRAL_API, get_key('analyzer1'), [
            ...
            'messages'    => [
                ['role'=>'system','content'=>"Résume en 3-5 phrases les informations clés sur cet utilisateur."],
